...
/// the tunnel.
pub const DIR: &str = "/etc/portman/wireguard";

/// The files under [`DIR`].
pub const PRIVATE_KEY_PATH: &str = "/etc/portman/wireguard/private.key";
pub const PUBLIC_KEY_PATH: &str = "/etc/portman/wireguard/public.key";
...
/// keys that belong to some *other* host - `portman add` mints a new
