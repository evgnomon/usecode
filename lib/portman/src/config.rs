//! Loads, validates, and mutates the portman configuration file. There
...
/// The well-known location for the portman config file.
pub const DEFAULT_PATH: &str = "/etc/portman/config.toml";

/// The local WireGuard interface settings. The private key is
/// deliberately not part of the config file: portman generates and
...
/// `client_address:local_port`"), set with `portman forward` /
/// `portman unforward`. Which one it is follows from whether
...
/// The root portman configuration.
#[derive(Debug, Default, Clone, Serialize, Deserialize)]
pub struct Config {
...
/// be added as a peer, via `portman export` / `portman import`. It never
...
const DEFAULT_TEMPLATE: &str = r#"# portman config - run "portman export" to fill this in, then "portman
...
impl Config {
    ...
    /// error asking the caller to run `portman export` first, rather
    ...
    pub fn load(path: &str) -> Result<Config> {
        if ensure_default(path)? {
            bail!(
                "no config found; wrote a template to {path} - run `portman export` to fill in \
                 this host's identity, then `portman import` to add peers"
            );
        }
    ...
    /// what `portman export` calls the first time it runs on a host.
    ...
    /// one exists (`portman forward` on an existing name repoints it
...
/// portman creates is root-only from the start; one that already exists
/// is left at whatever mode its owner chose (the Ansible role gives
/// /etc/portman 0750 so a group can list it).
