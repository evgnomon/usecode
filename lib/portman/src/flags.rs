//! A tiny command-line flag parser with the same surface portman's users
//! already type: single-dash long flags (`-config PATH`, `-force`),
//! `-flag=value` too, and parsing that stops at the first non-flag
//! argument so `portman forward NAME tcp 443` works unchanged.
...
mod tests {
    ...
    fn separate_and_inline_values_both_work() {
        ...
        fs.string("config", "/etc/portman/config.toml")
