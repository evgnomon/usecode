...
const PRE_CHAIN: &str = "PORTMAN_PRE"; // nat/PREROUTING: DNAT forward rules
const POST_CHAIN: &str = "PORTMAN_POST"; // nat/POSTROUTING: masquerade
const FWD_CHAIN: &str = "PORTMAN_FWD"; // filter/FORWARD: accept tunnel traffic
...
/// (Re)build portman's chains from `cfg`. Safe to call repeatedly; it
...
/// Remove portman's jump rules and chains, if present. A no-op (not an
...
/// portman's chains as text, for `portman status`.
pub fn ruleset() -> Result<String> {
    ...
    if b.is_empty() {
        bail!("no portman iptables chains found");
    }
    Ok(b)
}
...
mod tests {
    ...
    fn a_wildcard_bind_gets_no_destination_match() {
        ...
        assert!(rule.contains(&"portman:web".to_string()), "{rule:?}");
    }
