...
pub fn createVM(
    ...
    std.Io.Dir.cwd().createDir(io, cfg.vm_storage_path, .default_dir) catch |err| switch (err) {
        error.PathAlreadyExists => {},
        else => {
            std.log.err("Failed to create storage directory {s}: {}", .{ cfg.vm_storage_path, err });
            return err;
        },
    };
