...
/// Bash `<(echo "$data")`: a pipe fed from a thread.
pub fn data_sub(mut data: Vec<u8>) -> io::Result<(JoinHandle<()>, PipeReader)> {
    let (r, mut w) = io::pipe()?;
    data.push(b'\n');
...
mod tests {
    ...
    #[test]
    fn data_sub_appends_newline() {
        let (h, r) = data_sub(b"abc".to_vec()).unwrap();
        let mut cat = Command::new("cat");
        cat.arg(fd_path(&r));
        pass_fds(&mut cat, vec![r.as_raw_fd()]);
        let out = cat.output().unwrap();
