...
# Pulls container images from the registry deployed by
...
# registry's port before logging in and pulling.
#
# Usage: pull.sh [-s|--strip-host] <image>[:tag] [<image>[:tag] ...]
#
#   -s, --strip-host   after pulling, also tag the image without the
#                       registry host prefix (e.g. localhost:5000/foo -> foo)
#
...
usage() {
  echo "Usage: $0 [-s|--strip-host] <image>[:tag] [<image>[:tag] ...]" >&2
  exit 1
}
...
for IMAGE in "$@"; do
  SOURCE="${REGISTRY_ADDR}/${IMAGE}"
  echo "Pulling ${SOURCE}" >&2
  "$CONTAINER_CLI" pull "${TLS_ARGS[@]}" "$SOURCE"
  if [ "$STRIP_HOST" -eq 1 ]; then
    echo "Tagging ${SOURCE} -> ${IMAGE}" >&2
    "$CONTAINER_CLI" tag "$SOURCE" "$IMAGE"
  fi
done
